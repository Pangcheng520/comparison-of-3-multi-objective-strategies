...
def main():
    df = pd.read_excel("./strategies/HV_Feature.xlsx")
    # 如果有重复值，则保留第一个
    len1 = len(df.iloc[:, 0])
    ...
    df = pd.read_excel("./strategies/ElongationFeature.xlsx")
    # 如果有重复值，则保留第一个
    df.drop_duplicates(keep='first', inplace=True)
    df1 = df.copy()
    # 根据分位数准则删除异常值
    ...
    joblib.dump(lgbm_HV, './strategies/model/lgbm_feature20_HV.joblib')
    # 压缩应变模型
    ...
    joblib.dump(svr_EL, './strategies/model/svr_feature20_EL.joblib')
    ...
    algorithm = ea.moea_NSGA2_templet(
        ...
        MAXGEN=500,  # 最大进化代数，为了比较三种策略，尝试500
    ...
    # verbose=False是不显示日志信息,2024-2-27设置为True，其中评价指标HV越大越好，其他指标越小越好
    # 要保存结果文件时，将saveFlag的值改为True
    ...
    df_results = pd.DataFrame(res["Vars"])
    df_results["HV"] = res["ObjV"][:, 0]
    df_results["EL"] = res["ObjV"][:, 1]
    df_results.to_excel("./strategies/strategy1/NSGA2_feature_solve.xlsx",index=False)
